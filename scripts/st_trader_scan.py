...
def analyze_ticker(ticker: str, basis_price: float, shares: int):
    ...
    # ── Khuyến nghị tổng hợp (hoạt động với mọi nguồn dữ liệu) ────────────
    recommendation = "GIỮ"
    rec_reason = ""
    if "GIẢM" in trend or "XUỐNG" in trend:
        if pnl_pct < -7:
            recommendation = "BÁN — STOP LOSS"
            rec_reason = f"(P&L: {pnl_pct:.2f}%)"
        else:
            recommendation = "THEO DÕI — XU HƯỚNG Yếu"
    elif "TĂNG" in trend and "QUÁ MUA" in rsi_status:
        recommendation = "CẨN THẬN — RSI QUÁ MUA"
    elif "TĂNG" in trend:
        recommendation = "GIỮ — XU HƯỚNG TÍCH CỰC"
    elif "QUÁ BÁN" in rsi_status:
        recommendation = "THEO DÕI MUA — RSI QUÁ BÁN"
    ...
    report_path = os.path.join(REPORTS_DIR, f"{ticker}_st_scan_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md")
    md = [
    ...
    md.append(f"\n**Khuyến nghị hành động:** {recommendation} {rec_reason}")

...
def save_run_summary(results: dict, cfg: dict):
    ...
    summary = {
        ...
        "reports_count": len(os.listdir(REPORTS_DIR)) if os.path.isdir(REPORTS_DIR) else 0,
    }
...
def main():
    ...
    if not pdf_only:
        ...
        # Lưu ý: Cả 2 screeners (CANSLIM & VCP) đều sử dụng VNDataProvider
        # để truy xuất dữ liệu Việt Nam qua mozyfin/vnstock (không dùng FMP).
        if with_screeners:
            ...
            def normalize_symbol(sym: str) -> str:
                """Ensure symbol has exchange suffix for screener compatibility."""
                sym = sym.strip() if sym else ''
                if sym and '.' not in sym and len(sym) <= 5:  # VN ticker without suffix
                    return sym + '.VN'
                return sym

            symbols = set(
                normalize_symbol(item.get('ticker') or item.get('symbol') or '')
                for item in (list(active) + watchlist)
            )
            symbols.discard('')
            ...
            if symbols:
                ...
                scr_env = os.environ.copy()
                scr_env.update(env_overrides)
    ...
    # ── Bước 5: Dọn dẹp báo cáo cũ ────────────────────────────────────────────
    print("\n" + "="*50)
    print("Bước 5: Dọn dẹp báo cáo cũ...")
    cleanup_script = os.path.join(SCRIPTS_DIR, 'cleanup_reports.py')
    if os.path.exists(cleanup_script):
        ok5, _, _ = run_script('cleanup_reports.py', env=env_overrides)
        results["5. Cleanup Old Reports"] = ok5
    else:
        print("  [Bỏ qua] cleanup_reports.py chưa tồn tại.")
        results["5. Cleanup Old Reports"] = False

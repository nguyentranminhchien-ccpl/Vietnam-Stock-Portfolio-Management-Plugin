...
def generate_markdown_report(results: list[dict], metadata: dict, output_file: str):
    ...
    lines.append("")
    lines.append("> **Executive Summary - CANSLIM Screener:**")
    lines.append("> CANSLIM giúp tìm kiếm các cổ phiếu tăng trưởng có nền tảng cơ bản xuất sắc và đang thu hút dòng tiền. Trọng số: L (20%), A (20%), C (15%), S (15%), N (15%), I (10%), M (5%). Phù hợp để đánh theo chu kỳ tăng trưởng mạnh.")
    lines.append("")
    ...
    lines.append("> [!NOTE]")
    lines.append("> **Fact-Check Warning for Vietnam:** The CANSLIM metrics here are tuned for the US market.")
    lines.append("> 1. **Relative Strength (RS):** VNINDEX is highly concentrated. An RS Rank of 70+ in VN might be equivalent to 80+ in the US.")
    lines.append("> 2. **Institutional Sponsorship (I Component):** Data for Vietnamese institutional holdings is often opaque or delayed. Treat the 'I' score with caution.")
    lines.append("> 3. **Market Direction (M Component):** VNINDEX moves fast. Rely more on recent volume patterns rather than rigid 52-week moving averages.")
    lines.append("")
...
def format_stock_entry(rank: int, stock: dict) -> list[str]:
    ...
    n_breakout = "OK Breakout" if n_details.get("breakout_detected") else ""
    n_distance_str = (
    ...
    s_accumulation = "OK Accumulation" if s_details.get("accumulation_detected") else ""
...
if __name__ == "__main__":
    ...
    print("\nOK Test reports generated successfully")

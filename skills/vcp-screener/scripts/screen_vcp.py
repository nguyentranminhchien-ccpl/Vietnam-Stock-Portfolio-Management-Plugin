...
Screens VN-Index stocks for Mark Minervini's Volatility Contraction Pattern (VCP).
Uses a 3-phase pipeline: Pre-filter -> Trend Template -> VCP Detection & Scoring.

Usage:
    # Default (VN-Index, top 100 candidates, free tier API)
    python3 screen_vcp.py --api-key YOUR_KEY

    # Custom universe
    python3 screen_vcp.py --universe AAPL NVDA MSFT AMZN META

    # Full VN-Index (requires paid API tier)
...
def parse_arguments():
    base_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    plugin_dir = os.path.dirname(os.path.dirname(base_dir))
    default_output_dir = os.path.join(plugin_dir, "reports")
    ...
    parser.add_argument(
        "--full-sp500",
        action="store_true",
        help="Screen all VN-Index stocks (requires paid API tier, ~350 calls)",
    )
    parser.add_argument(
        "--mode",
    ...
    parser.add_argument(
        ...
        choices=[0, 1, 2],
        default=0,
        help="Relaxation level for pre-filter criteria (0: strict, 1: moderate, 2: loose)",
    )
    parser.add_argument(
        "--strict",
        action="store_true",
...
def passes_trend_filter(tt_result: dict, trend_min_score: int, relax_level: int = 0) -> bool:
    """Evaluate Minervini Trend Template."""
    # Adjust trend score threshold based on relax_level
    min_score = trend_min_score
    if relax_level == 1:
        min_score = max(0, trend_min_score - 10)  # Allow 10 points off
    elif relax_level == 2:
        min_score = max(0, trend_min_score - 20)  # Allow 20 points off
        
    return tt_result.get("raw_score", 0) >= min_score


def pre_filter_stock(quote: dict, relax_level: int = 0) -> tuple:
    """
    Cheap pre-filter using quote data only.

    Criteria (adjusted by relax_level):
    - Price > $10
    - At least X% above 52-week low
    - Within Y% of 52-week high
    - Average volume > 200,000

    Returns:
        (passed: bool, stage2_likelihood_score: float, is_transitioning: bool)
    """
    ...
    min_above_low = {0: 20, 1: 15, 2: 10}.get(relax_level, 20)
    max_below_high = {0: 30, 1: 40, 2: 50}.get(relax_level, 30)

    if pct_above_low < min_above_low:
        return False, 0, False
    ...
    if pct_below_high > max_below_high:
        return False, 0, False

    # Stage 2 score (0-100)
    stage2_score = 0
    if pct_above_low > 20: stage2_score += 20
    if pct_above_low > 30: stage2_score += 15
    if pct_below_high < 25: stage2_score += 30
    if pct_below_high < 15: stage2_score += 35
    
    is_transitioning = False
    if relax_level > 0 and pct_below_high > 30:
        is_transitioning = True

    return True, min(100, stage2_score), is_transitioning
...
def main():
    ...
    try:
        ...
        print("VNDataProvider initialized")
    except ValueError as e:
        print(f"ERROR: {e}", file=sys.stderr)
        sys.exit(1)
    ...
    if args.universe:
        ...
        print("  Fetching VN-Index constituents...", end=" ", flush=True)
        constituents = client.get_sp500_constituents()
        if not constituents:
            print("FAILED")
            print("ERROR: Unable to fetch VN-Index constituents", file=sys.stderr)
            sys.exit(1)
        symbols = [c["symbol"] for c in constituents]
        universe_desc = f"VN-Index ({len(symbols)} stocks)"
    ...
    quotes_list = client.get_quote(",".join(symbols))
    all_quotes = {q['symbol']: q for q in quotes_list} if quotes_list else {}
    ...
    for sym in symbols:
        ...
        if not quote:
            continue
        passed, likelihood, is_transitioning = pre_filter_stock(quote, relax_level=args.relax_level)
        if passed:
            pre_filtered.append((sym, likelihood, quote, is_transitioning))
    ...
    candidate_transitioning = {c[0]: c[3] for c in candidates}
    ...
    for sym, likelihood, quote, is_transitioning in candidates:
        ...
        # Use relax_level from args
        is_transitioning = candidate_transitioning.get(sym, False)
        tt_passed = passes_trend_filter(tt_result, args.trend_min_score, args.relax_level)
        
        if tt_passed or (args.relax_level > 0 and is_transitioning):
            if not tt_passed:
                is_transitioning = True # Force transitioning if it failed tt but passed relax
            elif args.relax_level > 0 and tt_result.get("raw_score", 0) < args.trend_min_score:
                is_transitioning = True
            trend_passed.append((sym, quote, is_transitioning))
    ...
    results = []
    for sym, quote, is_transitioning in trend_passed:
        hist = candidate_histories.get(sym, [])
        ...
        if analysis:
            if is_transitioning:
                analysis["execution_state"] = "Transitioning VCP"
                analysis["valid_vcp"] = False
    ...
    print()
    print("API Usage:")
    print(f"  API calls made: {api_stats.get('api_calls_made', 'N/A')}")
    print(f"  Cache entries:  {api_stats.get('cache_entries', 'N/A')}")
    print()

...
def generate_markdown_report(
    ...
    lines.append("")
    lines.append("> **Executive Summary - VCP Screener:**")
    lines.append("> VCP (Volatility Contraction Pattern) giúp tìm kiếm cổ phiếu đang kiến tạo nền giá siết chặt, rũ bỏ nguồn cung trước khi bứt phá. Phù hợp đánh sóng ngắn/trung hạn. Trọng số chấm điểm chính: Trend Template (xu hướng), Độ sâu VCP (siết nền), và Khối lượng cạn kiệt.")
    lines.append("")
    lines.append("---")
    lines.append("")
    ...
    section_b = [s for s in section_b if s.get("execution_state") != "Transitioning VCP"]
    ...
    # Section C: Transitioning VCP
    section_c = [s for s in results if s.get("execution_state") == "Transitioning VCP"]
    if section_c:
        count_label_c = f"{len(section_c)} stock{'s' if len(section_c) != 1 else ''}"
        lines.append(f"## Section C: Transitioning VCP ({count_label_c})")
        lines.append("")
        lines.append("> Tại thị trường Việt Nam, các nhịp rũ bỏ thường sâu hơn. Các cổ phiếu này có thể đang trong quá trình hình thành nền giá tích luỹ VCP ở biên độ rộng trước khi siết chặt.")
        lines.append("")
        for i, stock in enumerate(section_c, 1):
            lines.extend(_format_stock_entry(i, stock))
        lines.append("")
    ...
    lines.append("> [!NOTE]")
    lines.append("> **Fact-Check Warning for Vietnam:** Minervini's VCP is tuned for the US market.")
    lines.append("> 1. **Volatility Band:** VN-Index (HOSE) has a 7% daily limit, HNX 10%, UPCOM 15%. This creates unnatural ceilings/floors for volatility contractions. A tighter contraction than 3% might be artificial on HOSE.")
    lines.append("> 2. **Liquidity Filter:** Ensure the stock has sufficient liquidity before trusting the dry-up volume signals.")
    lines.append("")

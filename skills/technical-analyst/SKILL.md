...
**Filter Options (VCP):**
- **Default / Relaxed**: By default, point out early VCP signs and explain them in the context of the Vietnam market (as described in the framework).
- **Strict (--strict-vcp)**: If the user explicitly asks to apply the strict filter (e.g., passing `--strict-vcp`), ONLY identify VCP if it perfectly meets Minervini's textbook criteria (e.g., exact volume contraction, specific depth ratios). Do not point out early or loose signs.
